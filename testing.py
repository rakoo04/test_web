...
@app.route("/ask_answer_question", methods=['POST'])
def ask_answer_question():
    print(request.json)
    ...
    df['text'] = df.fname + ". " + hyperlink.remove_newlines(df.text)
    ...
    for row in df.iterrows():
       ...
       # If the number of tokens is greater than the max number of tokens, split the text into chunks
       if row[1]['n_tokens'] > max_tokens:
           shortened += hyperlink.split_into_many(row[1]['text'],tokenizer)
    ...
    # Note that you may run into rate limit issues depending on how many files you try to embed
    # Please check out our rate limit guide to learn more on how to handle this: https://platform.openai.com/docs/guides/rate-limits
    openai.api_key = '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'
    ...
    context = hyperlink.create_context(question,df,max_len=max_len,size=size,)
    #print(hyperlink.answer_question(df, question="What day is it?", debug=False))
